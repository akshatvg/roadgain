...
def visualize_file(filename, index_to_nodes):
    ...
    with open(filename) as f:
        ...
        for result in all_lines[1:]:
            ...
            true_results[node_str(index_to_nodes[int(NODE_A)], index_to_nodes[int(
                NODE_B)])] = float(SLOWDOWN.split("\\")[0])
    return true_results
...
def get_removable_streets(address, car_count=1000, streets_to_remove=200):
        ...
        with open(IN_FILE, 'w+') as f:
            ...
            for a, b in G.edges():
                ...
                if isinstance(lanes, list):
                    lanes = int(lanes[0])
                f.write('{} {} {} {}\n'.format(
                    nodes_to_index[a], nodes_to_index[b], G[a][b][0]['length'], lanes))

    # Original cost
    OUT_FILE_GEN = f'texts/{stripped_address}_{car_count}_gen.txt'
    overall = Popen(shlex.split(
        f'./cars {IN_FILE} {OUT_FILE_GEN} {car_count} -1 -1'))
    overall.wait()
    ...
    for (NODE_A, NODE_B, street_car_count, SLOWDOWN) in f_lines[:streets_to_remove]:
        ...
        processes.append(Popen(shlex.split(
            f'./cars {IN_FILE} {OUT_FILE} {car_count} {NODE_A} {NODE_B}')))
    ...
    for (NODE_A, NODE_B, street_car_count, SLOWDOWN) in f_lines[:streets_to_remove]:
        ...
        with open(OUT_FILE) as f:
            ...
            results.append(
                [round((cost_removed - total_cost) / total_cost, 3), node_str(NODE_A, NODE_B)])
            if cost_removed < total_cost+1111:
                to_remove.append(
                    node_str(index_to_nodes[int(NODE_A)], index_to_nodes[int(NODE_B)]))
...
def vals_to_colors(vals):
    ...
    ec = ['#{:02x}{:02x}{:02x}{:02x}'.format(
        *[int(colo * 255) for colo in cmap.to_rgba(cl)]) for cl in ev]
    return ec
...
def get_plot(address):
    ...
    cols = vals_to_colors([(orig_results[node_str(node_a, node_b)] if node_str(
        node_a, node_b) in orig_results else 0) for (node_a, node_b) in G.edges()])
    ...
    for i in cols:
        col2.append(i[:7])
    gdf_nodes, gdf_edges = ox.save_load.graph_to_gdfs(
